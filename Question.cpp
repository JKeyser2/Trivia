...
public:
  ...
  friend ostream& operator<< (ostream& output, Question<Y>& thingy);
...
template<class Y> //As friend function, has to use different templated variable
ostream& operator<< (ostream& output, Question<Y>& thingy){
    output << "Question: " << thingy.m_question << endl;
    output << "Please answer with a(n) " << thingy.m_datatype << endl;
    return output;
}
